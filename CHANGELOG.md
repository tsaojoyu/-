# Changelog — 花蓮港漁業文化網站 Demo

---

## v1.0 — 2026-05-19

### 新增
- **漁港區塊**完整互動邏輯
  - 四艘船隻色塊熱區（延繩釣船、拖曳網船、有鏢台的船、賞鯨船）
  - Hover 效果：translateY 上移 + 金色邊框光暈 + 船名 tooltip
  - Lightbox：點擊開啟、✕ 關閉、點擊遮罩關閉、ESC 關閉、焦點管理
  - 鍵盤可操作（Tab 選取熱區，Enter / Space 開啟）
- **木刻版畫風色彩系統**（`css/style.css` `:root` 區塊）
- **Banner 與漁場箱庭佔位區塊**（虛線框）
- CSS 色塊場景：天空漸層、山脈輪廓（遠近兩層）、海面水紋、碼頭

### 佔位素材
- 插畫：CSS 色塊與 clip-path 山形，非正式插畫
- 文字：`data/vessels.js` 四艘船說明與訪談節錄標示「佔位，待替換」

### 檔案結構
```
漁港demo/
  index.html          主頁面
  css/
    style.css         全域樣式、色彩變數
    harbor.css        漁港場景 + Lightbox 樣式
  js/
    harbor.js         熱區點擊與 Lightbox 邏輯
  data/
    vessels.js        四艘船隻資料（內容待替換）
  CHANGELOG.md
```

### 已知限制
- 無響應式設計（RWD），以寬螢幕（1200px+）瀏覽為主
- 漁場箱庭互動邏輯尚未實作
- 字型需網路連線載入（Noto Serif TC），離線自動退回系統字型

---

---

## v1.1 — 2026-05-19

### 新增
- **Lightbox 媒體區**
  - 影片播放器（`<video>` 原生播放，支援 .mp4；.MOV 需確認瀏覽器相容性）
  - 多張圖片輪播（左右箭頭切換、頁數計數）
  - 影片優先顯示，其次圖片；兩者都沒有則不顯示媒體區
  - 影片關閉 Lightbox 時自動停止
  - .MOV / 不支援格式顯示提示訊息
- `data/vessels.js` 新增 `images`（陣列）和 `video`（路徑或 null）欄位
  - 延繩釣船、有鏢台的船：各配置一支測試影片
  - 所有船隻：配置 picsum.photos 佔位圖片

### 媒體素材更新方式
- **替換照片**：將圖片放入 `images/` 資料夾，在 `vessels.js` 的 `images` 陣列填入路徑（如 `images/longline-1.jpg`）
- **替換影片**：將 .mp4 放入 `images/` 資料夾，在 `vessels.js` 的 `video` 欄位填入路徑
- **新增照片**：在 `images` 陣列新增一筆路徑即可，輪播自動調整
- **移除影片**：將 `video` 設為 `null`

---

## v1.2 — 2026-05-19

### 新增
- **漁場箱庭區塊**完整互動實作
  - 沿海示意插畫：天空、陸地（不規則海岸輪廓）、海面（近岸到深海漸層）
  - 四類熱區：山頭（七星潭、清水斷崖、鏢尾）、魚種（旗魚、曼波魚、煙仔虎）、潮流（沿岸流、南流、紅流）、船隻（漁民記憶三則）
  - 互動方式：點擊熱區 → 捲到下方卡片區，同類型卡片間左右翻頁
  - 卡片圖片輪播（山頭類別支援多張圖片）
  - 延伸閱讀區塊（Google Maps 漁場地圖、《看山版，聽流水》報告，URL 待填入）
  - ↑ 回到插畫按鈕

---

## v2.0 — 2026-05-20

### 架構重構：漁場箱庭互動模式全面改版

#### 互動邏輯
- **拆除單卡片面板**（舊：點擊才顯示、左右翻頁）
- **改為四大常駐橫條**，頁面載入即全部可見，讀者可自行往下捲動
- 插畫熱區改為**錨點導覽**：點擊後平滑捲動到對應橫條，並將目標卡片置中顯示、金色框線短暫閃爍提示
- 各橫條標題列加入「↑ 回到海上」按鈕

#### 類別更名
| 舊類別 | 新類別 |
|--------|--------|
| 山頭・看山版 | 漁場 |
| 魚種 | 魚種（不變）|
| 潮流 | 洋流 |
| 漁民・漁場記憶 | 漁法 |

#### 卡片軌道
- 橫向拖曳滑動，放開後有慣性飄動（速度 × 0.92 衰減）
- **景深效果**：中央卡片放大（scale 1.03）、上浮 10px、深陰影；邊緣卡片縮小（scale 0.91）、貼平、淺陰影；捲動時即時計算
- 目標卡片置中定位：點擊熱區時軌道自動水平捲到對應卡片

#### 視覺個性
- 四個類別各有獨立主色（標題列 + 卡片分隔線 + 引文邊框）
  - 漁場：墨綠（`--mountain-near`）
  - 魚種：海藍（`--sea-surface`）
  - 洋流：深海藍（`--sea-mid`）
  - 漁法：暖棕（`--dock-color`）

#### 佔位資料擴充（Demo 用）
- 各類別由 3 張卡擴充為 5–6 張，足以展示橫向拖曳與景深效果
- 新增佔位項目：美崙山、木瓜溪口、立霧溪口（漁場）；鬼頭刀、黑鮪魚、飛魚（魚種）；黑潮、湧升流（洋流）；延繩釣、鏢魚、定置漁網（漁法）

### 異動檔案
```
data/diorama.js   類別更名、佔位資料擴充
index.html        熱區 data-category 更新、舊卡片面板移除、橫條容器加入
css/diorama.css   全面重寫：新增橫條、軌道、景深、視覺個性樣式
js/diorama.js     全面重寫：橫條渲染、拖曳慣性、景深計算、錨點導覽
```

### 待辦（下一階段）
- [ ] 換入真實訪談文字（`data/diorama.js` 各類別 description / quote）
- [ ] 換入正式插畫後調整熱區位置（`index.html` 各熱區 style="" 數值）
- [ ] 延伸閱讀連結填入實際 URL（`data/diorama.js` DIORAMA_LINKS）
- [ ] 漁港換入真實訪談文字、照片、影片（`data/vessels.js`）
- [ ] 正式插畫定稿後調整漁港所有元素位置（`css/harbor.css`）

---

## v2.1 — 2026-05-20

### 漁港場景重構：元素大幅擴增

#### 船隻（3 艘，取代原本 4 艘）
| 新 | 原對應 | 說明 |
|----|--------|------|
| 出港漁船（鏢旗魚） | 有鏢台的船 | 船頭朝外海，暖棕色 |
| 進港漁船（流刺網） | 延繩釣船＋拖曳網船合併概念 | 帶漁獲返港，深藍綠 |
| 賞鯨船 | 賞鯨船 | 右側開放海域，不變 |

#### 新增：岸上標誌（6 個）
- 造船廠（深棕，最左，最高）
- 花蓮製冰冷凍廠（冷藍）
- 漁會（深色，最顯眼）
- 廣播電台 / 無線電（含天線裝飾）
- 台灣中油花蓮漁船站
- 燈塔（米白細柱，金色燈光，右端延伸至海面）

#### 新增：岸上行為（2 個，虛線框）
- 補網・整理漁具（碼頭中段）
- 港口順天宮・廣場（碼頭右段）

#### 技術調整
- 場景高度：520px → 600px
- 碼頭佔比：13% → 20%（容納建物色塊）
- 渲染順序：標誌與行為先渲染，船隻後渲染（確保船在建物前方）
- 船隻 z-index 提升至 6，標誌維持 5

#### 所有元素共用 Lightbox
所有 11 個元素皆使用 `.vessel-hotspot[data-vessel]` 機制，`harbor.js` 無需修改；`data/vessels.js` 已新增全部佔位說明文字。

### 異動檔案
```
data/vessels.js   重寫：3 艘船 + 6 個標誌 + 2 個行為佔位資料
index.html        漁港熱區全換（11 個元素）
css/harbor.css    場景高度、碼頭高度、新增標誌與行為樣式
```

---

## v2.2 — 2026-05-20

### 漁場箱庭互動模式改版：翻頁 + 淡入啟用

#### 閱讀模式
- 拖曳慣性滑動 → **翻頁模式**：每次顯示 2 張卡片，以 ← → 按鈕翻頁
- 每個橫條底部新增翻頁列（← 頁碼 →），頁碼顯示 `1 / 3` 格式
- 翻頁動畫：`translateX` + cubic-bezier 緩動，0.38s

#### 橫條啟用效果
- 預設所有橫條 `opacity: 0.4`（淡出）
- 捲動時偵測最接近視窗中央的橫條，自動 `opacity: 1`（啟用）
- 過渡時間 0.55s，流暢切換

#### 橫條間距
- 各橫條拉開 `margin-bottom: 2.5rem`，改為各自獨立邊框與陰影
- 橫條區塊頂端額外留白 `margin-top: 2.5rem`

#### 熱區點擊相容
- 點擊插畫熱區仍可跳到對應橫條，並跳到含目標卡片的分頁 + 金色閃爍

#### 技術備注
- 卡片寬度由 JS 動態設定（`track.offsetWidth / PER_VIEW`），支援視窗縮放
- 移除景深（scale/translateY）與拖曳邏輯，CSS 大幅簡化

### 異動檔案
```
js/diorama.js   全面改寫：翻頁邏輯、捲動啟用、resize 重算
css/diorama.css 全面改寫：翻頁模式、淡入樣式、nav 按鈕、橫條間距
```

---

## v2.3 — 2026-05-20

### 漁場箱庭互動模式改版：折疊手風琴 + v2.0 景深拖曳

#### 折疊手風琴
- 四大橫條預設**全部折疊**，只顯示彩色標題列，緊接拼在一起
- 點擊標題列展開（▶ 旋轉為 ▼）；**一次只能開一條**，點新的自動收起前一條
- 展開動畫：`height: 0 → auto`（精確測量 `scrollHeight`），0.42s ease
- 摺疊動畫：反向，同樣順滑
- 標題列「回到海上」按鈕：僅展開時顯示，點擊後折疊 + 捲回插畫

#### 展開後的閱讀模式（恢復 v2.0）
- 橫向拖曳 + 放開後慣性飄動（velocity × 0.92 衰減）
- 景深效果：中央卡 scale(1.03) 上浮 10px 深陰影；邊緣卡 scale(0.91) 貼平淺陰影

#### 熱區點擊相容
- 點擊插畫熱區 → 展開對應橫條 → 等展開動畫完成 → 目標卡片置中 + 金色閃爍

### 異動檔案
```
js/diorama.js   全面改寫：折疊邏輯、openStrip/closeStrip、景深＋拖曳恢復
css/diorama.css 全面改寫：body 折疊樣式、header 可點擊、▶ 指示符旋轉
```

---

## v2.4 — 2026-06-22

### 內容填入：正式資料全面替換佔位文字；插畫熱區擴充為全 20 個

#### vessels.js — 港口段落（全 11 個元素）
- 三艘船（出港/進港/賞鯨）、六個岸上標誌、兩個岸上行為，全部換入正式文字
- 訪談引言獨立至 `quote` 欄位：鏢魚師傅職業病、流刺網漁民海豚故事、漁民三支冰
- 廣播電台維持佔位文字（資料尚待補充）

#### diorama.js — 海上段落（全 20 張卡片）
所有類別完全以正式田野資料替換，舊佔位項目全數移除：

| 類別 | 項目（5 張） |
|------|------------|
| 漁場 | 溝灣仔、鏢尾、紅目鰱逝、龜仔口、ma-kong-kháu（皆附臺語羅馬拼音）|
| 魚種 | 立翅旗魚、大棘大眼鯛、赤鯥、康氏馬加鰆、鬼頭刀 |
| 洋流 | 紅流、南流、洘流、流東、流巡（皆附臺語羅馬拼音；流東附漁民諺語）|
| 漁法 | 延繩釣、流刺網、底刺網、拖網（待確認）、焚寄網 |

#### index.html — 插畫熱區：12 個 → 20 個
每類別由 3 個擴充為 5 個（全覆蓋）；位置採交錯排列避免標籤重疊

#### diorama.css — 插畫場景高度：500px → 700px
配合 20 個熱區提供足夠垂直空間

#### harbor.css / diorama.css — 新增 `white-space: pre-wrap`
讓多段落說明文字在 Lightbox 與卡片中正確換行顯示

### 異動檔案
```
data/vessels.js   全面更新：11 個元素換入正式文字（廣播電台除外）
data/diorama.js   全面重寫：20 張卡片換入正式研究資料
index.html        插畫熱區 12 → 20 個，標籤與 data-index 全部更新
css/diorama.css   場景高度 500px → 700px；新增 white-space: pre-wrap
css/harbor.css    新增 white-space: pre-wrap
```

### 待辦（下一階段）
- [ ] 廣播電台：補入正式文字（`data/vessels.js` radio.description）
- [ ] 延伸閱讀 URL：填入 Google Maps 漁場地圖、《看山版，聽流水》報告連結（`data/diorama.js` DIORAMA_LINKS）
- [x] 拖網資料確認 → 已更名為「叉手網」並換入正式說明（2026-06-22）
- [ ] 照片素材就位後填入路徑（`data/vessels.js` 各元素 images 陣列）
- [ ] 影片素材就位後填入路徑（`data/vessels.js` 各元素 video 欄位）
- [ ] 正式插畫定稿後調整所有熱區位置（`index.html` 各熱區 style="" 數值；`css/harbor.css` 各元素 left/top/width/height）

---

## v2.4.1 — 2026-06-22

### 漁法內容修正：拖網 → 叉手網

- `data/diorama.js`：將漁法類別 index 3 由「拖網」更名為「叉手網」，換入正式說明文字（以魩仔、鰻苗為主，冬季旺季，兩側長竿張網作業方式）
- `index.html`：對應插畫熱區標籤同步更新

---

## v2.5 — 2026-06-22

### Banner 手繪地圖輪播

#### 新增
- `js/banner.js`：5 張手繪地圖 crossfade 輪播（每張停留 5 秒，淡入淡出各 1.5 秒）
- `css/style.css`：`.banner-slideshow` / `.banner-slide` 樣式（opacity 疊在深海藍漸層上，透明度 0.28）
- `index.html`：Banner 加入 5 張 `<img class="banner-slide">` 與 `banner.js` 引入
- `images/map-1~5.jpg`：手繪漁場地圖圖片（高解析度 JPG）

#### 內容調整
- Banner 小標題移除「多羅滿蹲點 ×」，僅保留「花蓮港漁業文化」
- 延伸閱讀補上《看山版，聽流水》連結（`data/diorama.js` DIORAMA_LINKS）

#### 視覺微調
- 移除 Banner 橫線水紋裝飾（`::before` repeating-gradient）
- 地圖裁切方向：預設 `object-position: top`，map-1 / map-2 個別設定 `top right`

### 待辦（下一階段）
- [ ] 廣播電台：補入正式文字（`data/vessels.js` radio.description）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（`data/diorama.js` DIORAMA_LINKS[0].url）
- [ ] 照片素材就位後填入路徑（`data/vessels.js` 各元素 images 陣列）
- [ ] 影片素材就位後填入路徑（`data/vessels.js` 各元素 video 欄位）
- [ ] 正式插畫定稿後調整所有熱區位置
- [ ] 箱庭插畫物件（選項 B：20 張透明底 PNG + 1 張底圖，或改為整張交給插畫家）

---

## v2.6 — 2026-06-22

### 漁場地圖今昔比對區段

#### 新增
- `css/compare.css`：比對滑桿樣式（左右標籤、把手、導航箭頭）
- `js/compare.js`：比對滑桿互動邏輯，支援滑鼠、觸控、鍵盤；5 組箭頭切換
- `index.html`：新增 `#map-compare` 區段，延伸閱讀從 `#diorama` 內移出為獨立區塊
- `images/compare-old-1~5.jpg`：5 張手繪漁場地圖（已上傳）

#### 互動設計
- 左側：舊手繪地圖；右側：現代影像（現為深藍佔位，待換入船上拍攝照片）
- 下方 ← 1/5 → 箭頭切換五組比對
- `object-position: top` 防止地圖頂部被裁切

#### 素材資料夾建立
- `images/vessels/`：漁港 11 個元素的照片與影片
- `images/diorama/`：箱庭插畫物件（待用）

#### 素材命名規則確立
| 用途 | 路徑 | 命名 |
|------|------|------|
| 漁港照片 | `images/vessels/` | `outbound-01.jpg`、`inbound-01.jpg`… |
| 漁港影片 | `images/vessels/` | `outbound.mp4`… |
| 漁港場景底圖 | `images/` | `harbor-bg.jpg`（整張拼貼，取代色塊佔位） |
| 今昔比對舊地圖 | `images/` | `compare-old-1~5.jpg`（已就位） |
| 今昔比對現代影像 | `images/` | `compare-new-1~5.jpg`（待補） |

#### 決策紀錄
- 漁港場景插畫：採整張拼貼圖（`harbor-bg.jpg`），由使用者自行組合後匯出
- 箱庭插畫：委託插畫家，交付方式待確認（整張 or 個別物件）

### 待辦（下一階段）
- [ ] 廣播電台：補入正式文字（`data/vessels.js` radio.description）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（`data/diorama.js` DIORAMA_LINKS[0].url）
- [ ] 漁港場景底圖：拼貼完成後放入 `images/harbor-bg.jpg`
- [ ] 漁港照片素材：放入 `images/vessels/`，填入 `vessels.js` images 陣列
- [ ] 漁港影片素材：放入 `images/vessels/`，填入 `vessels.js` video 欄位
- [ ] 今昔比對現代影像：放入 `images/compare-new-1~5.jpg`，填入 `compare.js` pairs[].newSrc
- [ ] 箱庭插畫：確認交付方式後整合進網頁，調整熱區位置

---

## v2.7 — 2026-06-23

### 箱庭互動改版 + 漁法文字更新

#### 箱庭熱區重構（20 個 → 8 個）
- `index.html`：移除 20 個文字色塊熱區，改為每主題 2 個圖片熱區（`diorama-hotspot--img`）
- 熱區不再對應個別卡片，改為對應整個類別橫條
- 插畫未交付前顯示半透明虛線色塊佔位

#### 點擊行為改版
- `js/diorama.js`：點擊熱區展開整條橫條後，自動從第一張卡片捲動預覽至最後一張再回到起點（各 1 秒，中間停頓 0.3 秒），讓使用者看到共有 5 個項目
- 移除原本只高亮第一張卡片的 `is-highlighted` 邏輯與 header pulse CSS

#### 漁法文字更新（`data/diorama.js`）
- **延繩釣**：簡化為單段，移除釣鉤型號細節
- **流刺網**：措辭調整（「並在掙扎過程中與網纏繞」），移除 2021 年實名制段落
- **底刺網**：移除單層網／三層網分類段落
- **叉手網**：旺季後標點由句號改為逗號

#### 箱庭插畫規格（給設計師）
| 檔名 | 內容 | 尺寸 |
|------|------|------|
| `diorama/ground-1.png` | 漁場示意 1 | 600×800 px |
| `diorama/ground-2.png` | 漁場示意 2 | 600×800 px |
| `diorama/current-1.png` | 洋流示意 1 | 700×600 px |
| `diorama/current-2.png` | 洋流示意 2 | 700×600 px |
| `diorama/fish-1.png` | 魚種示意 1 | 800×400 px |
| `diorama/fish-2.png` | 魚種示意 2 | 800×400 px |
| `diorama/method-1.png` | 漁法示意 1 | 700×500 px |
| `diorama/method-2.png` | 漁法示意 2 | 700×500 px |

全部透明底 PNG，短邊不低於 400px。背景是否另繪待設計師確認。

### 待辦（下一階段）
- [ ] 廣播電台：補入正式文字（`data/vessels.js` radio.description）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（`data/diorama.js` DIORAMA_LINKS[0].url）
- [ ] 漁港場景底圖：拼貼完成後放入 `images/harbor-bg.jpg`
- [ ] 漁港照片素材：放入 `images/vessels/`，填入 `vessels.js` images 陣列
- [ ] 漁港影片素材：放入 `images/vessels/`，填入 `vessels.js` video 欄位
- [ ] 今昔比對現代影像：放入 `images/compare-new-1~5.jpg`，填入 `compare.js` pairs[].newSrc
- [ ] 箱庭插畫：8 張物件（+ 可選底圖）交付後整合，調整熱區位置
- [ ] Netlify 正式部署：v2.7 內容確認後擇機 push

---

## v2.8 — 2026-06-25

### 漁港插畫背景 + 標點式熱區 + 媒體素材大量補充

#### 漁港場景全面改版
- 換入等角插畫背景圖（`images/vessels/漁港bg.png`，1680×1292px）
- 移除所有 CSS 色塊圖層（天空/山/海/碼頭）
- 場景比例：2:1 → 4:3（配合插畫）
- 移除粗邊框，邊緣無框
- 漁港區段淡青色全寬背景（`#9ecfcf`），上下漸層柔邊過渡

#### 熱區改為標點式
- 11 個色塊/虛線熱區 → 10 個金色圓點標記（移除廣播電台）
- pulse 脈動動畫，hover 時放大 + 金色發光
- 名稱標籤常駐顯示（半透明深藍底），hover 變金色
- 補網・整理漁具標籤改為顯示在下方

#### 移除廣播電台
- 從 `index.html`、`data/vessels.js`、`css/harbor.css` 完整移除

#### 陰影移除
- 漁港 + 箱庭所有方塊陰影（`box-shadow: Npx Npx 0`）移除
- hover/active 發光效果保留

#### Lightbox 優化
- 說明標題「漁法介紹」→「內容介紹」
- 圖片移除粗邊框，改為自適應高度（無裁切無黑邊）
- 小標題改為「點擊金色標記，探索花蓮港的故事」

#### 箱庭新功能
- **影片浮動視窗**：卡片底部「▶ 播放影片」按鈕 → 全螢幕遮罩播放影片（ESC/點背景關閉）
- **圖片點擊放大**：點擊卡片圖片 → 全螢幕放大檢視
- 景深效果微調：最低透明度 45% → 70%、最低縮放 0.91 → 0.95

#### 媒體素材新增
**漁港照片（6 張）：**
| 元素 | 檔案 |
|------|------|
| 製冰冷凍廠 | `ice-1.jpg`、`ice-2.jpg` |
| 漁會 | `association.jpg` |
| 中油漁船站 | `oil-1.jpg`、`oil-2.jpg` |
| 順天宮 | `temple.jpg`（加入 temple-01.jpg 輪播）|

**漁港影片（1 支）：**
| 元素 | 檔案 |
|------|------|
| 出港漁船（鏢旗魚） | `outbound-harpoon.mp4` |

**箱庭魚種照片（5 張，Wikimedia Commons）：**
| 魚種 | 檔案 | 授權 |
|------|------|------|
| 立翅旗魚 | `striped-marlin.jpg` | CC0 |
| 大棘大眼鯛 | `red-bigeye.jpg` | CC BY-SA 4.0 |
| 赤鯥（紅喉） | `blackthroat-seaperch.jpg` | CC BY-SA 4.0 |
| 康氏馬加鰆 | `spanish-mackerel.jpg` | Public Domain |
| 鬼頭刀 | `mahi-mahi.jpg` | CC BY-SA 3.0 |

**箱庭漁法影片（2 支）：**
| 漁法 | 檔案 |
|------|------|
| 延繩釣 | `longline-mahimahi.mov` |
| 焚寄網 | `焚寄網.mov` |

#### 頁尾新增
- CC 圖片來源標註：「魚種照片來源：Wikimedia Commons（CC BY / CC BY-SA 授權）」

### 待辦（下一階段）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結
- [ ] 出港/進港漁船：替換 picsum 佔位圖為實際照片
- [ ] 造船廠：補照片
- [ ] 今昔比對現代影像：5 張右側照片
- [ ] 箱庭插畫：8 張物件 + 可選底圖
- [ ] 視覺風格改版：白底 + 無襯線 + 留白（等素材齊全後執行）
- [ ] 響應式設計
- [ ] Netlify 正式部署

---

## v2.9 — 2026-06-26

### 漁港新寬圖、熱區重新定位、Lightbox 標籤客製化

#### 漁港背景換新寬圖
- 插畫換為 `harbor bg_wider.png`（2560×1440，16:9）
- `harbor.css` 的 `aspect-ratio` 更新為 `2560 / 1440`

#### 熱區座標重新定位
- 10 個熱區依新寬圖重新標點定位

#### Lightbox 標籤客製化
- `vessels.js` 各條目新增 `descriptionLabel` 欄位
- `harbor.js` 動態帶入，取代固定的「內容介紹」
- 各熱區標籤：鏢魚的技藝／返航的人／產業轉型／船的醫院／保鮮之道／漁民的後盾／出海前的準備／地標故事／日常一景／文化背景

#### 訪談節錄區塊動態顯示
- `quote` 為 `null` 的熱區，Lightbox 內「訪談節錄」區塊自動隱藏

### 待辦（下一階段）
- [x] 延伸閱讀 URL：《看山版，聽流水》已填入
- [x] 出港/進港漁船：替換 picsum 佔位圖為實際照片
- [x] 造船廠：補照片
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（仍為 #）
- [ ] 今昔比對現代影像：5 張右側照片
- [ ] 視覺風格改版：白底 + 無襯線 + 留白（等素材齊全後執行）
- [ ] 響應式設計
- [ ] Netlify 正式部署

---

## v3.0 — 2026-06-29

### 漁場箱庭場景全面改版（seascape）+ 漁港素材大量補入

#### 漁場箱庭插畫場景（seascape）
- 移除舊色塊佔位場景（`.diorama-scene`），換為全新插畫版（`.seascape`）
- 場景由插畫家去背 PNG 多層疊加組成：背景圖、洋流×5、魚種×5、漁法說明
- 熱區改為不可見 `<button class="seascape-hotspot">`，hover 對應圖層以 `is-lit` 亮起
- 漁場改為金色標點式（`.seascape-ground-marker`），與漁港標點風格一致
- 新增座標工具（點擊插畫顯示 left/top %，`display:none` 預設隱藏，需用時開啟）

#### 箱庭卡片更新
- 魚種卡片圖片：Wikimedia Commons 照片 → 插畫家 PNG（`旗魚-new.png` 等）
- 移除卡片圖片點擊放大功能（`.diorama-img-modal` 整組移除）
- 卡片圖片背景由深棕（`#1a1510`）改為淡藍（`#daedf6`），配合插畫色調

#### 新增 JS 精準跳卡（`openStripAndGoToCard`）
- seascape 熱區點擊 → 展開對應橫條 → 定位到指定卡片 → 金色閃爍提示

#### 漁港素材補充
| 類型 | 檔案 | 補充說明 |
|------|------|----------|
| 出港漁船照片 | `出港漁船-01.jpg`、`出港漁船-02.jpg` | 取代 picsum 佔位 |
| 進港漁船照片 | `進港漁船-01.jpg`、`進港漁船-02.jpg` | 取代 picsum 佔位 |
| 進港漁船影片 | `進港漁船 流刺網收網(記得靜音).mp4` | 靜音播放（`videoMuted: true`） |
| 造船廠照片 | `造船廠-01.jpg` | 新補 |
| 賞鯨船照片 | `whale-02.jpg` | 第二張 |

#### 其他
- `harbor.js` 新增 `videoMuted` 支援
- `vessels.js` 進港漁船補入第二張照片（`進港漁船-02.jpg`）

### 待辦（下一階段）
- [x] seascape 熱區微調：確認所有標點位置後移除座標工具
- [x] 鏢尾漁場標點：補入 seascape（`data-index="1"`）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（`data/diorama.js` DIORAMA_LINKS[0].url）
- [ ] 今昔比對現代影像：5 張右側照片（`images/compare-new-1~5.jpg`）
- [ ] 視覺風格改版：白底 + 無襯線 + 留白
- [ ] 響應式設計
- [ ] Netlify 正式部署

---

## v3.1 — 2026-06-29

### 視覺收尾：框線移除 + 漁場標點完整

#### 移除漁場箱庭所有框線
- `.diorama-strip` 外框（3px）移除，統一與漁港區段無框風格
- `.diorama-strip-card` 卡片間右側分隔線（2px）移除
- `.diorama-strip-card-media` 圖片區塊外框（2px）移除
- `.diorama-deep-links` 延伸閱讀外框（3px）及項目分隔線（2px）移除
- `.diorama-strip-img-counter` 圖片計數列上邊框（1px）移除

#### 漁場標點補齊（共 5 個）
- 補入「鏢尾」（`data-index="1"`），座標 `left:58.5%; top:36.3%`
- 座標工具完整移除（HTML / JS / CSS）

### 待辦（下一階段）
- [ ] 延伸閱讀 URL：Google Maps 漁場地圖連結（`data/diorama.js` DIORAMA_LINKS[0].url）
- [ ] 今昔比對現代影像：5 張右側照片（`images/compare-new-1~5.jpg`）
- [ ] 視覺風格改版：白底 + 無襯線 + 留白
- [ ] 響應式設計
- [x] Netlify 部署：v3.1 上線（2026-06-29）
